...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({
  ...
  return (
    ...
    >
      <Markdown remarkPlugins={[remarkGfm, remarkMath]} components={components}>
        {isStreaming ? processedTextRef.current : safeMarkdownText}
      </Markdown>
      {(isStreaming || (!isStreaming && !processedTextRef.current)) && (
        <span className="markdown-cursor">▋</span>
      )}
    </div>
  );
};
